...
document.getElementById('platformTabs').addEventListener('click', (e) => {
  ...
  const name = PLATFORM_NAMES[state.platform] || state.platform;
  document.getElementById('quickPublishText').textContent = `${name} 바로 발행`;
  document.getElementById('quickCopyText').textContent = `${name} 복사 후 이동`;
});
...
function handleQuickAction(action) {
  if (action === 'publish') {
    publishNow(state.platform);
  } else {
    copyAndAlert(state.platform);
  }
}
...
async function generateContent() {
  ...
  try {
    ...
    updateCharCounter();
    showToast('✨ 원고 생성 완료! 이제 위쪽 미디어 에디터에서 사진/영상을 채워보세요', 'success');

    // "당신은 데이터만 넣으면 됩니다" — 다음 단계(미디어 에디터)로 스무스하게 앵커 스크롤
    scrollToMediaEditor();
...
function scrollToMediaEditor() {
  const target = document.getElementById('mediaEditorAnchor');
  target.scrollIntoView({ behavior: 'smooth', block: 'start' });
  target.classList.remove('anchor-enter', 'anchor-flash');
  // 리플로우를 강제해 애니메이션이 다시 재생되도록 함
  void target.offsetWidth;
  target.classList.add('anchor-enter', 'anchor-flash');
}
...
function scrollToSchedule() {
  document.getElementById('scheduleAnchor').scrollIntoView({ behavior: 'smooth', block: 'center' });
}
...
function applyPreset(text, el) {
  ...
  showToast('📝 주제가 입력되었습니다. 아래에서 AI 생성을 눌러보세요!', 'info');
  document.getElementById('topicInput').focus();
  document.getElementById('topicPanel').scrollIntoView({ behavior: 'smooth', block: 'center' });
}
